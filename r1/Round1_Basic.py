...
class Status:
    ...
    def hist_vwap_all(self, size: int) -> np.ndarray:
        ...
        for i in range(1, 4):
            tmp_bid_vol = self.hist_order_depth('bidamt', i, size)
            tmp_ask_vol = self.hist_order_depth('askamt', i, size)
            tmp_bid_prc = self.hist_order_depth('bidprc', i, size)
            tmp_ask_prc = self.hist_order_depth('askprc', i, size)
...
class Trade:
    ...
    def rainforest_resin(state: Status) -> list[Order]:
        ...
        orders.extend(Strategy.mm_ou(state=state, sigma = 0.001, fair_price=current_price, gamma=0.2, order_amount=50))
    ...
    def kelp(state: Status) -> list[Order]:
        reversion_factor = -.25
        best_ask = state.maxamt_askprc
        best_bid = state.maxamt_bidprc

        # Ensure there is sufficient order book data
        if best_ask is None or best_bid is None:
            return None

        # Compue the mid-market price
        mid_price = (best_ask + best_bid) / 2

        # Retrieve the historical mid prices (assumes a reasonable default history size, e.g., 10)
        historical_mid_prices = state.hist_mid_prc(size=10)

        # Apply the reversion formula if historical data is present
        if len(historical_mid_prices) > 1:
            last_price = historical_mid_prices[-1]
            returns = (mid_price - last_price) / last_price
            predicted_returns = returns * reversion_factor
            adjusted_fair_price = mid_price + (mid_price * predicted_returns)
        else:
            adjusted_fair_price = mid_price
        orders = []
        current_price = adjusted_fair_price

        orders = []
        orders.extend(Strategy.arb(state=state, fair_price=current_price))
        orders.extend(Strategy.mm_glft(state=state, fair_price=current_price, gamma=0.2, order_amount=50))
    ...
    def squid_ink(state: Status) -> list[Order]:
        ...
        orders = []
        # for the strat parameters liek sigma and gamma, we can change them adaptively.
        orders.extend(Strategy.arb(state=state, fair_price=current_price))
        orders.extend(Strategy.mm_glft(state=state, fair_price=current_price, gamma=0.2, order_amount=50)) # old gamma = 0.1
        return orders
